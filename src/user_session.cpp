...
namespace worms_server
{
    ...
    disconnect_user(const std::shared_ptr<user>& client_user)
    {
        ...
        if (const auto game =
                database->get_game_by_name(client_user->get_name()))
        {
        ...
        const auto packet_bytes = worms_packet::freeze(
            packet_code::disconnect_user, {.value10 = client_user->get_id()});
        for (const auto& user : database->get_users())
        {
    ...
    user_session::user_session(ip::tcp::socket socket) :
        database_(database::get_instance()), socket_(std::move(socket)),
        timer_(socket_.get_executor()), strand_(socket_.get_executor())
    {
            ...
            while (socket_.is_open())
            {
                try
                {
                    ...
                    const size_t read = co_await socket_.async_receive(
                        buffer(incoming), redirect_error(use_awaitable, ec));
                    ...
                    while (true)
                    {
                        ...
                        spdlog::debug(
                            "Received packet code {} from {}",
                            static_cast<uint32_t>(data.value()->code()),
                            username);

                        if (!co_await packet_handler::handle_packet(
                                user_, database_, std::move(*data)))
                        {
                            spdlog::warn(
