...
namespace worms_server
{
    ...
    uint32_t database::get_next_id()
    {
        ...
        if (uint32_t recycled_id;
            instance->recycled_ids_.try_dequeue(recycled_id))
            return recycled_id;

        uint32_t id =
            instance->next_id_.fetch_add(1, std::memory_order::relaxed);
        if (id == 0 || id < k_start)
        {
            id = k_start;
    ...
    database::get_users_in_room(const uint32_t room_id) const
    {
    ...
    std::shared_ptr<game>
    database::get_game_by_name(std::string_view name) const
    {
        const std::shared_lock lock(games_mutex_);

        auto view = games_ | std::views::values;
        const auto it =
            std::ranges::find_if(view, [name](const auto& game)
                                 { return game->get_name() == name; });
                                    ...
                                    const uint32_t room_id)
    {
        ...
        const auto it =
            std::ranges::find_if(users_, [user_id](const auto& user) -> bool
