...
namespace worms_server
{
                                      ...
                                      uint32_t left_id)
    {
        ...
        const auto room_leave_packet_bytes = worms_packet::freeze(
            packet_code::leave, {.value2 = room_id, .value10 = left_id});
        const auto room_close_packet_bytes =
            worms_packet::freeze(packet_code::close, {.value10 = room_id});
    ...
    static awaitable<bool>
    on_chat_room(const std::shared_ptr<user>& client_user,
                 const std::shared_ptr<database>& database,
                 const worms_packet_ptr& packet)
    {
        if (packet->fields().value0.value_or(0) != client_user->get_id() ||
            !packet->fields().value3 || !packet->fields().data)
        {
        ...
        if (message.starts_with(std::format("GRP:[ {} ]  "sv, client_name)))
        {
            ...
            if (client_room_id == target_id)
            {
                ...
                const auto packet_bytes =
                    worms_packet::freeze(packet_code::chat_room,
                                         {.value0 = client_id,
                                          .value3 = client_room_id,
                                          .data = message.data()});

                for (const auto& user : database->get_users())
                {
                    if (user->get_room_id() == client_room_id &&
                        user->get_id() != client_id)
                    {
                ...
                client_user->send_packet(worms_packet::freeze(
                    packet_code::chat_room_reply, {.error = 0}));
                co_return true;
            }

            // Notify sender
            client_user->send_packet(worms_packet::freeze(
                packet_code::chat_room_reply, {.error = 1}));
            co_return true;
        }
        ...
        if (message.starts_with(std::format("PRV:[ {} ]  "sv, client_name)))
        {
            ...
            if (target_user == nullptr ||
                target_user->get_room_id() != client_room_id)
            {
                client_user->send_packet(worms_packet::freeze(
                    packet_code::chat_room_reply, {.error = 1}));
                co_return true;
    ...
    static awaitable<bool>
    on_list_rooms(const std::shared_ptr<user>& client_user,
                  const std::shared_ptr<database>& database,
                  const worms_packet_ptr& packet)
    {
    ...
    static awaitable<bool>
    on_list_users(const std::shared_ptr<user>& client_user,
                  const std::shared_ptr<database>& database,
                  const worms_packet_ptr& packet)
    {
        if (packet->fields().value4.value_or(0) != 0 ||
    ...
    static awaitable<bool>
    on_list_games(const std::shared_ptr<user>& client_user,
                  const std::shared_ptr<database>& database,
                  const worms_packet_ptr& packet)
    {
        if (packet->fields().value4.value_or(0) != 0 ||
    ...
    static awaitable<bool>
    on_create_room(const std::shared_ptr<user>& client_user,
                   const std::shared_ptr<database>& database,
                   const worms_packet_ptr& packet)
    {
        if (packet->fields().value1.value_or(0) != 0 ||
        ...
        if (std::ranges::any_of(database->get_rooms(),
                                [requested_room_name](const auto& room) -> bool
                                {
                                    return equals_case_insensitive(
                                        requested_room_name, room->get_name());
                                }))
        {
            client_user->send_packet(worms_packet::freeze(
                packet_code::create_room_reply, {.value1 = 0, .error = 1}));
        ...
        const auto room_packet_bytes =
            worms_packet::freeze(packet_code::create_room,
                                 {.value1 = room_id,
                                  .value4 = 0,
                                  .name = std::string(room->get_name()),
                                  .data = "",
        ...
        client_user->send_packet(worms_packet::freeze(
            packet_code::create_room_reply, {.value1 = room_id, .error = 0}));
    ...
    static awaitable<bool> on_join(const std::shared_ptr<user>& client_user,
                                   ...
                                   const worms_packet_ptr& packet)
    {
        if (!packet->fields().value2 ||
            packet->fields().value10.value_or(0) != client_user->get_id())
        {
            spdlog::error("Invalid packet data");
            co_return false;
        }
                ...
                { return room->get_id() == join_id; }))
        {
            ...
            const auto packet_bytes =
                worms_packet::freeze(packet_code::join,
        ...
        if (std::ranges::any_of(
                ...
                [join_id = *packet->fields().value2,
                 room_id = client_user->get_room_id()](const auto& game) -> bool
                {
                    return game->get_id() == join_id &&
                        game->get_room_id() == room_id;
                }))
        {
            // Notify other users about the join.
    ...
    static awaitable<bool>
    on_create_game(const std::shared_ptr<user>& client_user,
                   const std::shared_ptr<database>& database,
                   const worms_packet_ptr& packet)
    {
        ...
        if (parsed_ip.is_v4() && client_user->get_address() == parsed_ip)
        {
            ...
            // Send reply to host;
            client_user->send_packet(
                worms_packet::freeze(packet_code::create_game_reply,
                                     {.value1 = game_id, .error = 0}));
        }


        client_user->send_packet(worms_packet::freeze(
            packet_code::create_game_reply, {.value1 = 0, .error = 2}));
        client_user->send_packet(worms_packet::freeze(
    ...
    static awaitable<bool>
    on_connect_game(const std::shared_ptr<user>& client_user,
                    const std::shared_ptr<database>& database,
                    const worms_packet_ptr& packet)
    {
        ...
        const auto room_id = client_user->get_room_id();
        const auto it =
            std::ranges::find_if(games,
                                 [game_id, room_id](const auto& game) -> bool
                                 {
                                     return game->get_id() == game_id &&
                                         game->get_room_id() == room_id;
                                 });
        ...
        {
            client_user->send_packet(worms_packet::freeze(
                packet_code::connect_game_reply, {.data = "", .error = 1}));
        }
        else
        {
            client_user->send_packet(worms_packet::freeze(
    ...
    awaitable<bool>
    packet_handler::handle_packet(const std::shared_ptr<user>& client_user,
                                  const std::shared_ptr<database>& database,
                                  const worms_packet_ptr& packet)
    {
        switch (packet->code())
