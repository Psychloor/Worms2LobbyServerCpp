...
namespace worms_server
{
    ...
    bool session_info::verify_session_info(const session_info& info)
    {
        ...
        {
            spdlog::error("CRC Missmatch - CRC1: {} CRC2: {}", info.crc1,
                          info.crc2);
            return false;
        }
        ...
        if (info.always_one != 1 || info.always_zero != 0)
        {
            spdlog::error(
                ...
                info.always_one, info.always_zero);
            return false;
        }

        const auto result =
            std::ranges::find_if_not(info.padding, [](const auto& value)
                                     { return value == net::byte{0}; });
