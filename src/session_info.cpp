...
namespace worms_server {
    session_info::session_info(
        const worms_server::nation nation, const session_type type, const session_access access) {
    ...
    net::deserialization_result<session_info, std::string> session_info::read_from(net::packet_reader& reader) {
        session_info info;

        info.crc1          = *reader.read_le<uint32_t>();
        info.crc2          = *reader.read_le<uint32_t>();
        info.player_nation = static_cast<worms_server::nation>(*reader.read_le<uint8_t>());
    ...
    bool session_info::verify_session_info(const session_info& info) {
        ...
        if (info.always_one != 1 || info.always_zero != 0) {
            spdlog::error(
                "Always one/zero mismatch - Always one: {} Always zero: {}", info.always_one, info.always_zero);
            return false;
        }
